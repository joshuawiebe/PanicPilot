...
class Slider:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        hx = self._handle_x()
        filled = pygame.Rect(self._track.left, self._track.top, hx - self._track.left, 8)
        ...
        surface.blit(val_lbl, (self.cx + self.width // 2 + 14, self.cy - val_lbl.get_height() // 2))
...
class TextInput:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        if self.active and pygame.time.get_ticks() % 1000 < 500:
            ...
            pygame.draw.line(surface, C_TEXT, (x, self.rect.y+8), (x, self.rect.bottom-8), 2)
...
class Menu:
    ...
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        self._btn_client = Button(cx, y0 + BTN_H + BTN_GAP, "  CLIENT VERBINDEN  ")
        self._btn_quit   = Button(cx, y0 + 2*(BTN_H + BTN_GAP), "  BEENDEN  ",
                                  w=200, h=46)
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                ...
                if (event.type == pygame.KEYDOWN
                        and event.key == pygame.K_ESCAPE): return "quit"
...
class HostSetupMenu:
    ...
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        try:
            with _sock.socket(_sock.AF_INET, _sock.SOCK_DGRAM) as _s:
                _s.connect(("8.8.8.8", 80)); self._own_ip = _s.getsockname()[0]
        except OSError:
            self._own_ip = _sock.gethostbyname(_sock.gethostname())
        cx = SCREEN_W // 2
        self._slider   = Slider(cx, SCREEN_H // 2 - 60, "Streckenlänge (Tiles)", 10, 50, 20)
        self._modes    = [1, 2, 3]
        self._mode_idx = 0
        self._speed_idx = 1   # Standard = Normal
        self._mode_labels = {1: "Modus 1: Split Control",
    ...
    def run(self, prefill: dict | None = None) -> tuple | None:
        ...
        while True:
            ...
            pygame.draw.rect(self.screen, (15, 30, 60), box.inflate(20, 10), border_radius=6)
            pygame.draw.rect(self.screen, C_BTN_BORDER, box.inflate(20, 10), 2, border_radius=6)
            ...
            self.screen.blit(m_lbl, ((SCREEN_W - m_lbl.get_width()) // 2,
                                      SCREEN_H // 2 - 10))
...
class ClientSetupMenu:
    ...
    def run(self) -> str | None:
        ...
        while True:
            ...
            self.screen.blit(lbl, ((SCREEN_W - lbl.get_width()) // 2, SCREEN_H // 2 - 70))
            self._input.draw(self.screen)
            hint = self._hint.render(
                "Die IP des Hosts steht im Fenster-Titel des Hosts.", True, C_LABEL)
            self.screen.blit(hint, ((SCREEN_W - hint.get_width()) // 2, SCREEN_H // 2 + 28))
    ...
    while True:
        ...
        if choice == "quit":
            ...
            if result is not None:
                ...
                last_settings = {"mode": mode, "length": length,
